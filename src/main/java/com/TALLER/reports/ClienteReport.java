...
public class ClienteReport {
	private List<mCliente> listaclientes;
	
	private XSSFWorkbook libro;
	private XSSFSheet hoja;

	public ClienteReport(List<mCliente> listaclientes) {
		this.listaclientes = listaclientes;
		this.libro = new XSSFWorkbook();
		this.hoja = libro.createSheet("Clientes");
	}
		...
		Font fuente=FontFactory.getFont(FontFactory.TIMES_ROMAN);
		   ...
		   for(mCliente cliente:listaclientes){
			   if(cliente.getEstado().equals("ACTIVO")) {
				    celda.setPhrase(new Phrase(cliente.getNombres()+" "+cliente.getApellidos(),fuente));
				   	tabla.addCell(celda);
				   	celda.setPhrase(new Phrase(cliente.getDni(),fuente));
				   	tabla.addCell(celda);
				   	celda.setPhrase(new Phrase(cliente.getDistrito().getNombre(),fuente));
				   	tabla.addCell(celda);
				   	celda.setPhrase(new Phrase(cliente.getDireccion(),fuente));
				   	tabla.addCell(celda);
				   	celda.setPhrase(new Phrase(cliente.getTelefono(),fuente));
				   	tabla.addCell(celda);
				   	celda.setPhrase(new Phrase(cliente.getEmail(),fuente));
				   	tabla.addCell(celda); 
			   }
	...
	public void exportar(HttpServletResponse response) throws DocumentException, IOException{
		...
		tabla.setWidths(new float[]{6f,2f,3f,6f,2.8f,5.5f});
	    ...
	    for(mCliente cliente:listaclientes) {
	    	if(cliente.getEstado().equals("ACTIVO")) {
	    		Row fila = hoja.createRow(numeroFilas++);
	    		 Cell celda = fila.createCell(0);
	    		 celda.setCellStyle(estilo);
	    		 celda.setCellValue(cliente.getNombres()+" "+cliente.getApellidos());
	    		 ...
	    		 celda = fila.createCell(2);
	    		 celda.setCellStyle(estilo);
	    		 celda.setCellValue(cliente.getDistrito().getNombre());
